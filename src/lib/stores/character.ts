...
function createCharacterStore() {
	...
	const derivedStore = derived(store, ($character) => {
		...
		return {
			...
			isConcentrationBreaking: (spell: Spell) =>
				spell.concentration && !!$character.currentConcentration,
			isUnpreparedRitual: (spell: Spell) =>
				!$character.preparedSpellsIds.includes(spell.name) && spell.ritual,
